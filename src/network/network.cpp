...
char server[] =
    "172.20.10.2";  // IP address of the server you want to connect to
...
const int bufferSize =
    1024;  // Adjust the buffer size based on your message size
...
void registration() {
  ...
  String jsonMessage =
      "{\"id\":" + String(robot_id) + ",\"command\":\"register\"}";
...
void handleCommand(String command, float data) {
  if (command == "move") {
    ...
    // TODO: modify the moveforward so that it takes the distance into account +
    // do we want to make it move forward after that directly??
    turn_angle(data);
  }
}